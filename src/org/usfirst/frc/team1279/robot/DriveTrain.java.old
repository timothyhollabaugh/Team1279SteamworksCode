...
public class DriveTrain implements Constants, MotorSafety {
	 ...
	 */
	public DriveTrain(final int frontLeftMotor, final int rearLeftMotor,
		...
		// group Left Front side and configure
		//m_frontLeftMotor
		...
		// group Left Rear side and configure
		//m_rearLeftMotor
		//		.setFeedbackDevice(CANTalon.FeedbackDevice.CtreMagEncoder_Relative);
		//m_rearLeftMotor
		//		.changeControlMode(CANTalon.TalonControlMode.PercentVbus);
		...
		m_rearLeftMotor.changeControlMode(CANTalon.TalonControlMode.Follower);
		m_rearLeftMotor.set(frontLeftMotor);


		// group Right Front side and configure
		//m_frontRightMotor
		//		.setFeedbackDevice(CANTalon.FeedbackDevice.CtreMagEncoder_Relative);
		...
		// group Right Rear side and configure
		//m_rearRightMotor
		//		.setFeedbackDevice(CANTalon.FeedbackDevice.CtreMagEncoder_Relative);
		//m_rearRightMotor
		//		.changeControlMode(CANTalon.TalonControlMode.PercentVbus);
		...
		m_rearRightMotor.changeControlMode(CANTalon.TalonControlMode.Follower);
		m_rearRightMotor.set(frontRightMotor);
	 ...
	 */
	public DriveTrain(CANTalon frontLeftMotor, CANTalon rearLeftMotor,
			CANTalon frontRightMotor, CANTalon rearRightMotor) {
		m_frontLeftMotor = requireNonNull(frontLeftMotor,
				"frontLeftMotor cannot be null");
		m_rearLeftMotor = requireNonNull(rearLeftMotor,
				"rearLeftMotor cannot be null");
		m_frontRightMotor = requireNonNull(frontRightMotor,
				"frontRightMotor cannot be null");
		m_rearRightMotor = requireNonNull(rearRightMotor,
				"rearRightMotor cannot be null");
		m_sensitivity = kDefaultSensitivity;
		m_maxOutput = kDefaultMaxOutput;
	 ...
	 */
	public void tankDrive(GenericHID stick, final int leftAxis,
			final int rightAxis) {
		if (stick == null) {
			throw new NullPointerException("Null HID provided");
		}
	 ...
	 *
	 * @param stick
	 *            Joystick object to use for the left and right side of the
	 *            robot.
	 * @param leftAxis
	 *            The axis to select on the left side Joystick object.
	 * @param rightAxis
	...
	public void tankDrive(GenericHID stick, final int leftAxis,
		...
		tankDrive(stick.getRawAxis(leftAxis), stick.getRawAxis(rightAxis),
				squaredInputs);
	}
	...
	public void setLeftRightMotorOutputs(double leftOutput, double rightOutput) {
		if (m_rearLeftMotor == null || m_rearRightMotor == null
				|| m_frontLeftMotor == null || m_frontRightMotor == null) {
		...
		System.out.println(Timer.getFPGATimestamp() + ": DriveLeft:"
				+ leftOutput + " DriveRight:" + rightOutput);
	}
	 ...
	 * drive would be inverted, the drive code assumes that the motors are
	 * geared with one reversal.
	 */
	public void reverseDirection() {
		m_isInverted = !m_isInverted;
		System.out.println("DirectionInverted:"
				+ Boolean.toString(m_isInverted));
	...
	public void autoDistance(double speed, double distance) {
		...
		while (DriverStation.getInstance().isAutonomous()) {
			...
			if (count <= 100)
				...
				adj = 1.0;
			if (m_rearLeftMotor.getEncPosition() <= -d
					&& m_rearRightMotor.getEncPosition() >= d) {
			...
			System.out.println(Timer.getFPGATimestamp() + ":RENC:"
					+ m_rearRightMotor.getEncPosition() + " LENC:"
					+ m_rearLeftMotor.getEncPosition());
		}
	}
