...
public class GearClaw {
  ...
  /**
   * The current at which to trigger going into current control mode to hold the gear
   */
  public static final double CLAW_TRIGGER_CURRENT = 0.2;
  ...
  public boolean isClosedEnough(){
    ...
    SmartDashboard.putString("DB/String 0", Double.toString(clawTalon.getOutputCurrent()));
    SmartDashboard.putString("DB/String 1", Double.toString(clawTalon.getPosition()));
    SmartDashboard.putString("DB/String 2", Double.toString(clawTalon.getOutputVoltage()));
		if(!open){
		  if(!hasGear){
        if(clawTalon.getOutputCurrent() >= CLAW_TRIGGER_CURRENT){
          if(clawTalon.getPosition() < CLAW_GEAR_MIN_POS){
            ...
            // There is a gear
            SmartDashboard.putString("DB/String 3", "Found a Gear!");
            this.hasGear = true;
